...
\midrule
Accelerator & \% & $\sim$219k & 100 & \textit{(from run)} & \textit{(from run)} & 0--100 \\
Speed & km/h & $\sim$219k & 50$\rightarrow$100 & \textit{(from run)} & \textit{(from run)} & 0--$\sim$80 \\
\bottomrule
\end{tabular}\\[0.3em]
\small\textit{Note: Exact values depend on which A2D2 files are loaded. The pipeline prints and logs all statistics.}
\end{table}

\begin{figure}[H]
\centering
\fbox{\parbox{0.92\textwidth}{\centering\vspace{5cm}
\textbf{[PLACEHOLDER -- Figure~\ref{fig:a2d2_signals}]}\\[0.5em]
Use the plot generated by the notebook: \texttt{part1\_a2d2\_comprehensive.png}\\
This shows: (a) time-series of accelerator and speed for the first 30\,s, (b) distribution histograms, (c) box plots, (d) correlation scatter plot, (e) metadata table.
\vspace{0.5cm}}}
...
\midrule
healthy.csv & Calibration & -- & -- & 9,000 \\
acc fault gain.csv & Test & Accelerator & Gain & 31,500 \\
acc fault noise.csv & Test & Accelerator & Noise & 32,200 \\
acc fault stuck.csv & Test & Accelerator & Stuck-at & 31,700 \\
rpm fault gain.csv & Test & Speed & Gain & 32,300 \\
rpm fault noise.csv & Test & Speed & Noise & 31,700 \\
rpm fault stuck at.csv & Test & Speed & Stuck-at & 32,700 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.92\textwidth}{\centering\vspace{6cm}
\textbf{[PLACEHOLDER -- Figure~\ref{fig:fusion_plot}]}\\[0.5em]
Generate a figure with two rows:\\
\textbf{Top row:} Speed signal from A2D2 (healthy) for the first 60\,s.\\
\textbf{Bottom row:} Speed signal from ``rpm fault gain.csv'' (HIL), with the fault range highlighted using a red shaded region. Annotate the fault start and end times.\\
Both plots should share the same x-axis scale and y-axis label. Use the title: ``Dataset Fusion: A2D2 Healthy Training Data vs.\ HIL Fault Test Data (Speed Sensor).''\\
This directly addresses the supervisor's request to ``show the data set fusion (A2D2 with HIL) and highlight the fault range in the plot, e.g., present the rpm signal.''
\vspace{0.5cm}}}
...
\centering
\fbox{\parbox{0.92\textwidth}{\centering\vspace{5cm}
\textbf{[PLACEHOLDER -- Figure~\ref{fig:augmentation_examples}]}\\[0.5em]
Use the plot generated by the notebook: \texttt{part2\_augmentation\_examples.png}\\
Shows a single training window and its three augmented versions (jittered, scaled, masked) for both sensor channels.
\vspace{0.5cm}}}
...
\textbf{[PLACEHOLDER -- Figure~\ref{fig:training_loss}]}\\[0.5em]
Use the plot generated by the notebook: \texttt{part2\_training\_curves.png}\\
Shows the training loss curve over all steps and the per-epoch average loss. The loss should decrease rapidly in the first 10 epochs and then converge gradually.
\vspace{0.5cm}}}
