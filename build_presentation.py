...
tb = add_textbox(s, L, T_CONTENT, Inches(5.8), Inches(5.5))
tf = tb.text_frame; tf.word_wrap = True
set_text(tf, "A2D2 \u2014 Training Data (Phase 1)", size=18, bold=True, color=DARK_GREEN)
add_bullet(tf, "Audi real-world driving, Munich", size=15)
add_bullet(tf, f"{R['a2d2']['total_samples']:,} samples ({R['a2d2']['duration_sec']/60:.1f} min)", size=15)
add_bullet(tf, "Sensors: Accelerator (%), Speed (km/h)", size=15)
add_bullet(tf, "Accelerator: 100 Hz native", size=15)
add_bullet(tf, "Speed: 50 Hz \u2192 upsampled to 100 Hz", size=15)
add_bullet(tf, "3 recording sessions combined", size=15)
add_bullet(tf, "Only healthy data used for training", size=15, bold=True)
add_para(tf, "", size=8)
add_para(tf, "A2D2 Statistics:", size=16, bold=True, color=MED_BLUE)
add_bullet(tf, f"Acc pedal: \u03bc={R['a2d2']['acc_mean']:.2f}%, \u03c3={R['a2d2']['acc_std']:.2f}%, range [0, {R['a2d2']['acc_max']:.0f}]%", size=13)
add_bullet(tf, f"Speed: \u03bc={R['a2d2']['spd_mean']:.2f} km/h, \u03c3={R['a2d2']['spd_std']:.2f}, range [0, {R['a2d2']['spd_max']:.1f}]", size=13)
add_bullet(tf, f"Correlation: {R['a2d2']['correlation']:.4f}", size=13)

tb = add_textbox(s, Inches(6.8), T_CONTENT, Inches(5.8), Inches(5.5))
tf = tb.text_frame; tf.word_wrap = True
set_text(tf, "HIL \u2014 Test Data (Phases 2-3)", size=18, bold=True, color=DARK_RED)
add_bullet(tf, "dSPACE HIL simulator, TU Clausthal", size=15)
add_bullet(tf, "1 healthy + 6 fault recordings", size=15)
add_bullet(tf, "3 fault types \u00d7 2 sensors = 6 scenarios", size=15)
add_bullet(tf, f"Calibration: {R['healthy']['n_windows']} healthy windows (90 sec)", size=15)
add_bullet(tf, "Cross-domain transfer: real \u2192 simulation", size=15, bold=True)
add_para(tf, "", size=8)
add_para(tf, "HIL Fault Files:", size=16, bold=True, color=MED_BLUE)
add_bullet(tf, "acc fault gain.csv (314 windows)", size=13)
add_bullet(tf, "acc fault noise.csv (321 windows)", size=13)
add_bullet(tf, "acc fault stuck.csv (315 windows)", size=13)
add_bullet(tf, "rpm fault gain.csv (321 windows)", size=13)
add_bullet(tf, "rpm fault noise.csv (315 windows)", size=13)
add_bullet(tf, "rpm fault stuck at.csv (325 windows)", size=13)
add_para(tf, "", size=6)
add_para(tf, "Total fault windows: 1,911", size=15, bold=True, color=DARK_RED)
...
add_para(tf, "", size=6)
add_bullet(tf, f"Total pipeline: ~{total:.0f} s (~{total/60:.1f} min)", size=16)
add_para(tf, "", size=6)
...
add_para(tf, f"  Our approach: ~{23000/total:.0f}x faster", size=16, bold=True, color=DARK_GREEN)
